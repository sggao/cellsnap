...
cell_nbhd = np.load(os.path.join(load_path, "cell_nbhd_res0.5_k20_forimg.npy")) # default
...
image_folder = os.path.join(load_path2, "processed_data", "single_cell_images", f"size{size}")
...
class CNN_cell256_channel2(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.fc3 = nn.Linear(128, cell_nbhd.shape[1])
...
train_size, test_size = train_nbhd.shape[0], test_nbhd.shape[0]
model_save_path = os.path.join(load_path, "cnn", f"cnn_256_l{l}_layer6_testimgsize:256_checkpoints", "epochs")
