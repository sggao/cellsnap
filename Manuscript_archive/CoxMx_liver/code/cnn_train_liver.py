...
# read data and save train maks
df = pd.read_csv("../data/livcan_meta_y4_x69.csv")
rna_df = pd.read_csv('../data/livcan_counts_y4_x69.csv', index_col = 0)

print([df.shape, rna_df.shape])

## save train maks:
train_mask = np.array([False] * df.shape[0])
train_size = int(0.8 * df.shape[0])
train_index = np.random.choice(df.shape[0], train_size, replace=False)
train_mask[train_index] = True
np.save('../data/469_train_mask.npy', train_mask)
...
train_mask = np.load(os.path.join(metaload_path, "469_train_mask.npy"))
...
image_folder = '../data/single_cell_images/HCC_4_69/'
dataset = SingleCellImageDataset_Stream(image_folder, train_mask, cell_nbhd, use_transform = True)
...
class CNN_cell512_channel2_layer6(nn.Module):
    def __init__(self):
        ...
        self.fc4 = nn.Linear(128, 11) # change due to cell type labels
...
model_save_path = os.path.join(metaload_path, "469_cnn", f"cnn_512_l{l}_layer6", "epochs")
